...
MODIF 2026-07 : plancher ATR sur le SL de fibo_volume (mode "floor" validé
par backtest 3 ans : aucune année négative, contre -14R en 2026 avec
l'ancien SL). Le SL garde sa logique d'origine (low/high de la bougie
+/- 0.1*range) mais sa distance ne peut plus être inférieure à 1.0*ATR(14),
ce qui élimine les micro-SL sous le spread réel.
"""
...
FIBO = dict(N_BINS=30, POC_ZONE_BINS=1, BODY_MAX=0.35, SL_BUFFER=0.10,
            ...
            ATR_FLOOR=1.0)   # distance SL minimale en multiples d'ATR(14)


def add_common(df):
    """Ajoute EMA, ATR et pivots fractals. À appeler une fois par cycle."""
    df = df.copy()
        ...
        if setup:
            ...
            if setup["side"] == "buy":
                if row["close"] < setup["lo"]:
                    ...
                    entry = row["close"]
                    # SL d'origine avec plancher ATR (mode "floor" du backtest)
                    sl = min(row["low"] - P["SL_BUFFER"] * rng,
                             entry - P["ATR_FLOOR"] * row["atr"])
                    risk = entry - sl
                    if risk > 0 and i == last:
                        return {"side": "buy", "entry": entry, "sl": sl,
                                "tp": entry + P["RR"] * risk}
                ...
                if row["close"] > setup["hi"]:
                    ...
                    entry = row["close"]
                    # SL d'origine avec plancher ATR (mode "floor" du backtest)
                    sl = max(row["high"] + P["SL_BUFFER"] * rng,
                             entry + P["ATR_FLOOR"] * row["atr"])
                    risk = sl - entry
                    if risk > 0 and i == last:
                        return {"side": "sell", "entry": entry, "sl": sl,
