...
MIN_RISK_PCT   = 0.05    # distance SL minimale en % du prix (0.05 = 2$ sur un actif
                         # a 4000$) ; filtre les micro-trades sous le spread reel.
                         # Mettre 0 pour retrouver le comportement d'origine.
...
# Variantes OTE, comparees dans un meme run (cascade : chaque niveau ajoute un levier)
#   candle  : version d'origine                                        — reference
#   floor   : plancher de distance SL a 1.0*ATR(14)
#   rr2     : floor + TP vise 2.0R (breakeven theorique d'un WR de 33%)
#   session : floor + entrees uniquement 07h-20h UTC (Londres + New York)
#   hivol   : floor + entrees uniquement si ATR/prix > sa mediane 10 jours
OTE_VARIANTS = {
    "candle":  {},
    "floor":   {"floor": 1.0},
    "rr2":     {"floor": 1.0, "rr": 2.0},
    "session": {"floor": 1.0, "hours": (7, 20)},
    "hivol":   {"floor": 1.0, "vol_min": True},
}
...
# Variantes de SL pour fibo-volume, comparees dans un meme run :
#   candle : SL d'origine (low/high de la bougie +/- 0.1*range)      — reference
#   floor  : SL d'origine avec plancher de distance a 1.0*ATR(14)
#   atr    : SL a 1.5*ATR(14) de l'entree (standard scalping 5m)
#   poc    : SL a 0.5*ATR(14) au-dela de la zone POC (invalidation volume)
FIBO_SL_MODES = ["candle", "floor", "atr", "poc"]
ATR_MULT   = 1.5   # mode atr
ATR_FLOOR  = 1.0   # mode floor
POC_BUFFER = 0.5   # mode poc, en multiples d'ATR
...
class Engine:
    def __init__(self, bot, ts_labels):
        ...
        self.rejected = 0         # trades ecartes par le filtre MIN_RISK_PCT
    ...
    def try_open(self, i, side, entry, sl, tp):
        ...
        if entry > 0 and risk / entry * 100 < MIN_RISK_PCT:
            self.rejected += 1        # micro-trade : SL plus proche que le spread reel
            return
...
# ----------------------------- 1. OTE Scalping --------------------------------
def bt_ote(df, eng, variant="candle"):
    P = OTE
    V = OTE_VARIANTS[variant]
    rr_mult = V.get("rr", P["RR"])
    ...
    if V.get("vol_min"):
        ...
        vol_ok = (ratio > ratio.rolling(2880).median()).values   # mediane ~10 jours
    ...
    def final_sl(side, i, entry, zone_sl):
        if not V.get("floor"):
            return zone_sl
        if side == "sell":
            return max(zone_sl, entry + V["floor"] * atr[i])
        return min(zone_sl, entry - V["floor"] * atr[i])
        ...
        if zone:
            if zone["side"] == "sell":
                ...
                if c[i] > zone["sl"]:
                    ...
                    entry = c[i]
                    sl = final_sl("sell", i, entry, zone["sl"])
                    risk = sl - entry
                    if risk > 0 and allowed(i):
                        eng.try_open(i, "sell", entry, sl, entry - rr_mult * risk)
                    zone = None
            else:
                if l[i] <= zone["f618"]:
                    zone["touched"] = True
                if c[i] < zone["sl"]:
                    ...
                    entry = c[i]
                    sl = final_sl("buy", i, entry, zone["sl"])
                    risk = entry - sl
                    if risk > 0 and allowed(i):
                        eng.try_open(i, "buy", entry, sl, entry + rr_mult * risk)
                    zone = None
...
def bt_fibo(df, eng, sl_mode="candle"):
    P = FIBO
    o, h, l, c = (df[k].values for k in ("open", "high", "low", "close"))
    e = df["ema"].values
    v = df["vol"].values
    ...
    def stop_for(side, i, entry, rng, poc_lo, poc_hi):
        if side == "buy":
            base = l[i] - P["SL_BUFFER"] * rng
            if sl_mode == "candle":
                return base
            if sl_mode == "floor":
                return min(base, entry - ATR_FLOOR * atr[i])
            if sl_mode == "atr":
                return entry - ATR_MULT * atr[i]
            return poc_lo - POC_BUFFER * atr[i]            # poc
        base = h[i] + P["SL_BUFFER"] * rng
        if sl_mode == "candle":
            return base
        if sl_mode == "floor":
            return max(base, entry + ATR_FLOOR * atr[i])
        if sl_mode == "atr":
            return entry + ATR_MULT * atr[i]
        return poc_hi + POC_BUFFER * atr[i]                # poc
        ...
        if setup:
            ...
            if setup["side"] == "buy":
                if c[i] < setup["lo"]:
                    ...
                    entry = c[i]
                    sl = stop_for("buy", i, entry, rng, setup["poc_lo"], setup["poc_hi"])
                    risk = entry - sl
                    if risk > 0:
                ...
                if c[i] > setup["hi"]:
                    ...
                    entry = c[i]
                    sl = stop_for("sell", i, entry, rng, setup["poc_lo"], setup["poc_hi"])
                    risk = sl - entry
                    if risk > 0:
...
def main():
    ...
    runs = [(f"ote-{m}", bt_ote, m) for m in OTE_VARIANTS]
    runs += [("fvg-ifvg", bt_fvg, None)]
    runs += [("fibo-candle", bt_fibo, "candle"), ("fibo-floor", bt_fibo, "floor")]
    rows, yrows = [], []
    for bot, fn, mode in runs:
        print(f"\n[backtest] {bot} en cours...")
        eng = Engine(bot, ts_labels)
        fn(dfc, eng) if mode is None else fn(dfc, eng, mode)
    ...
    yearly.to_csv(os.path.join(OUT_DIR, "ventilation-annuelle.csv"), index=False)
    print(f"\n===== SYNTHESE 3 ANS (filtre MIN_RISK_PCT = {MIN_RISK_PCT}%) =====")
